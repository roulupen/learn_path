...
from app.utils.logger import loggers, log_llm_request, log_llm_response, log_llm_error, log_performance_metric
...
class GeminiAIService:
    ...
    async def generate_study_plan(
        ...
        if not self._is_available():
            logger.warning("Gemini AI not available, using fallback study plan")
        ...
        try:
            ...
            # Log the request
            log_llm_request(gemini_logger, full_prompt, user_context)
            ...
            log_performance_metric(
                performance_logger,
                'study_plan_generation_time',
                processing_time,
                {'course': course_title, 'duration_weeks': duration_weeks}
            )
            
            logger.info(f"Successfully generated AI study plan for course: {course_title} in {processing_time:.2f}s")
            gemini_logger.info(f"STUDY_PLAN_SUCCESS: Generated plan with {len(study_plan.get('modules', []))} modules")
            
            return study_plan
            
        except Exception as e:
            processing_time = time.time() - start_time
            log_llm_error(gemini_logger, e, prompt if 'prompt' in locals() else None, user_context)
            log_performance_metric(
                performance_logger,
                'study_plan_generation_error',
                processing_time,
                {'course': course_title, 'error': str(e)}
            )
            logger.error(f"Error generating study plan for {course_title}: {str(e)}")
            return self._fallback_study_plan(course_title, user_level, duration_weeks)
